	...
	<div class="detail">
		<div class="detail-row">
    		...
    		<div class="row-input" style="width:60%;" >
    			<form id="add">
	    			<input type="text" name="named" placeholder="请输入文章类型名称" maxlength="24">
	    			{{ csrf_field() }}
	    		</form>
    		</div>			
		</div>
...
class active{
	...
	deleteActive(id){
		if ( !confirm('确定删除该文章分类吗？') ) {
			return;
		}
		$.ajax({
			url:'/article/deleteArticleType/'+id,
			type:'get',
			error:()=>{
				parent.$.warn('服务器忙');
			},
