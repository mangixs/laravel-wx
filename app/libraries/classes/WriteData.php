...
class WriteData {
    ...
    public function phpData($file, $name, $data) {
        $file    = $this->create_path($file);
        $fp      = fopen($file . '/' . $name . '.php', 'a+');
        $content = "<?php \n\r $" . $name . '=' . var_export($data, true) . " \n\r ?>";
        fwrite($fp, $content);
        fclose($fp);
    }
    ...
    public function put_content_file($path, $name, $data, $ext = null) {
        $file = $this->create_path($path);
        $dir  = '/' . $name;
        if ($ext) {
            $dir .= ('.' . $ext);
        }
        file_put_contents($file . $dir, $data);
        return $path . $dir;
    }
